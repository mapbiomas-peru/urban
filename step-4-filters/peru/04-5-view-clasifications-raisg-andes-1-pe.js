...
var inclusion = /* color: #3614d6 */ee.FeatureCollection(
    ...
    exclusion = /* color: #ff0000 */ee.FeatureCollection(
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-78.5807975061964, -5.674363145022699],
                  [-78.57496101938, -5.651643608679556],
                  [-78.5807975061964, -5.6500207504968065],
                  [-78.60671837411633, -5.692982248169038],
                  [-78.59916527353039, -5.695117427905651]]]),
            {
              "value": 1,
              "type": "exclusion",
              "system:index": "2"
            })]),
    geometry = /* color: #03ffb8 */ee.Geometry.MultiPoint(
        [[-78.42680547773577, -5.733537842593517],
         [-78.5164127165053, -5.6378804142997465],
         [-78.44225500166155, -5.611571845952546],
         [-78.6321124845717, -5.676145292599175],
         [-78.58130071699358, -5.709283358649578],
         [-78.69837377607561, -5.650521770058194],
         [-78.76806829511858, -5.661112963803345]]);
...
var param = {
  ...
  versionClassification: '6',   // Area de clasificacion con el Buffer 
  no_piramide: true,
...
var Urban = function(param){
  ...
  this.inputs = {
    ...
    _regions : 'projects/mapbiomas-raisg/DATOS_AUXILIARES/VECTORES/clasificacion-regiones-urban-4',
    ...
    shpIncluExclu: 'projects/mapbiomas-raisg/MUESTRAS/'+param.country+'/COLECCION4/TRANSVERSALES/URBANA/STEP3_GEOMETRY/urban-'+ param.region + '-' + param.country + '-' + param.classificationArea.shpVersion,
    samples: 'projects/mapbiomas-raisg/MUESTRAS/'+param.country+'/COLECCION4/TRANSVERSALES/URBANA/',
    result: 'projects/mapbiomas-raisg/TRANSVERSALES/'+param.country+'/COLECCION4/URBANA/clasificacion-ft',
  ...
  this.init = function(param){
    ...
    inclusionSHP = inclusionSHP.merge(param.classificationArea.inclusion);
    exclusionSHP = exclusionSHP.merge(param.classificationArea.exclusion);
   }
      
   var GeometriesIE = ee.FeatureCollection([inclusionSHP, exclusionSHP]).flatten()
    // print('GeometriesIE',GeometriesIE)
    
    // Classification
    var classified = ee.ImageCollection(outputPathClasif).filter(ee.Filter.eq('code_region', regionId))
    ...
    if(param.exportassets.classificationArea){
      Export.image.toAsset({
        ...
        assetId:'projects/mapbiomas-raisg/MUESTRAS/'+country+'/COLECCION4/TRANSVERSALES/URBANA/STEP1_REGIONS/'+ 'URBANA-'+ 'REF-ACCUM' + '-'+ country + '-'+ regionId + '-'+ param.classificationArea.versionClassAreaOutput,
        scale: 30,
        pyramidingPolicy: {
          '.default': 'mode'
        },
    ...
    var assetId = 'projects/mapbiomas-raisg/MUESTRAS/'+country+'/COLECCION4/TRANSVERSALES/URBANA/STEP3_GEOMETRY/' + fileName;
