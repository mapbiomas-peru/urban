...
var geometry = /* color: #3ed627 */ee.Geometry.MultiPoint();
/***** End of imports. If edited, may not auto-convert in the playground. *****/
var param = { 
    code_region: 30408,  //Region de Clasificacion 
    pais: 'COLOMBIA', 
    year: 2022,  // Solo visualizacion
    version_input: '2', // revisar version de input de clasificacion 
    version_output: '3',
...
var ordem_exec_first =  [27];              //Filtro de primer año
var ordem_exec_last =   [24];                               //Filtro de ultimo año
var ordem_exec_middle = [24,27,24,27];  //Filtro de años intermedios

//--------------------------------------------

var dirinput = 'projects/mapbiomas-raisg/TRANSVERSALES/'+param.pais+'/COLECCION4/URBANA/clasificacion-ft'
var dirout  =   'projects/mapbiomas-raisg/TRANSVERSALES/'+param.pais+'/COLECCION4/URBANA/clasificacion-ft/'
...
bandNames.getInfo().forEach(
  function (bandNames) {
    ...
    var band0 = ee.Image(0).updateMask(regionRaster)
    band0 = band0.where(image.gte(0),image)
    classif = classif.addBands(band0.rename(bandNames))
  }
)
Clasificacion_TD =classif.select(bandNames).selfMask();
...
var mask3last = function(valor, imagem){
  ...
  img_out = img_out.addBands([imagem.select('classification_1986'),
                              ...
                              imagem.select('classification_2020'),
                              imagem.select('classification_2021')]);
  img_out = img_out.addBands(imagem.select('classification_2022').blend(muda_img));
  return img_out;
};
