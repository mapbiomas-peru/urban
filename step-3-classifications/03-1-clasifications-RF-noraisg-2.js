...
var inclusion = /* color: #3614d6 */ee.FeatureCollection(
    ...
    geometry_27 = /* color: #b1b1b1 */ee.FeatureCollection(
        [ee.Feature(
    ...
    geometry_24 = /* color: #ff0000 */ee.FeatureCollection(
        [ee.Feature(
        ...
        ee.Feature(
            ee.Geometry.Point([-78.77933528162728, 1.8063493304479834]),
            {
              "id": 24,
              "system:index": "2"
            })]),
    Muestra24_1985_1990 = /* color: #d63000 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Point([-70.13600381970699, -15.490875845736246]),
            {
              "reference": 24,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Point([-70.1314892920514, -15.488872973065652]),
            {
              "reference": 24,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-70.12993445974226, -15.491184138851432],
                  [-70.12873283010359, -15.48804103640541],
                  [-70.13517013173933, -15.492011263131873]]]),
            {
              "reference": 24,
              "t0": 1985,
              "t1": 1990,
    ...
    Muestra27_1985_1990 = /* color: #cc19ff */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Point([-70.13068231702144, -15.470858399589345]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Point([-70.11815103650386, -15.472016486441246]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Point([-70.1586631214648, -15.49319178883527]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-70.15606990438337, -15.49622957974244],
                  [-70.15233626943464, -15.495071594191586],
                  [-70.14924636190165, -15.494492615519368],
                  [-70.15096297841902, -15.49341736681307],
                  [-70.16014686208601, -15.495071594191586]]]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1990,
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-70.1158582268321, -15.497552910432853],
                  [-70.11680236440534, -15.495154305212962],
                  [-70.11748900991316, -15.503011701276941]]]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1990,
    ...
    Muestra27_1985 = /* color: #98ff00 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-70.1407491264903, -15.485187388856453],
                  [-70.13946166616316, -15.483450366721334],
                  [-70.14649978261824, -15.480886164527188],
                  [-70.15122047048445, -15.484608383099038]]]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1985,
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-70.12083640676374, -15.477081160762882],
                  [-70.12632957082624, -15.473937844008274],
                  [-70.12933364492292, -15.47939725834456],
                  [-70.1265012322032, -15.480968881223145]]]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1985,
        ...
        ee.Feature(
            ee.Geometry.Point([-70.11648236601759, -15.48528560040212]),
            {
              "reference": 27,
              "t0": 1985,
              "t1": 1985,
...
//Apoyo NICFI
// var fecha1 = '2022-01-01'
// var fecha2 = '2022-12-31'

var param = {
  region: 30412,
  country: 'COLOMBIA',
  trees: 70,
  yearsPreview: [2000, 2022],
  _print: true,
  tileScale: 8,
  inputVersionSample: '1',
  outVersionClass: '1',
  GlobalSurfaceWater: 30,  // 0-100 indicar la frecuencia para enmascarar Agua
  additionalSamples: { // MUESTRAS ESTABLES 
    polygons: [ geometry_24, geometry_27],
    classes: [ 24, 27 ], // clases en orden de las geometrias
    points: [500, 300]   //Numero de muestras a agregar en cada clase
  },
  ...
  classificationArea : {   // Para Incluir o exluir el Area de clasificacion con el Buffer 
        ...
        inclusion: inclusion, //Agrega área de trabajo a classArea
        exclusion: exclusion, //Excluye área de trabajo a classArea
        useshp: true,
        shpVersion: '1'
  }
};
...
var Urban = function(param){
  ...
  this.inputs = {
    ...
    shpIncluExclu: 'projects/mapbiomas-raisg/MUESTRAS/'+param.country+'/COLECCION5/TRANSVERSALES/URBANA/STEP2_GEOMETRY/urban-'+ param.region + '-' + param.country + '-' + param.classificationArea.shpVersion,
    years: [
      // 1985, 1986, 1987, 1988, 
      // 1989, 1990, 1991, 1992, 
      // 1993, 1994, 1995, 1996, 
...
var RGB = {bands:['R','G','B'],"min":64,"max":1900,"gamma":0.74};
// var nicfi = ee.ImageCollection('projects/planet-nicfi/assets/basemaps/americas')
//             .filter(ee.Filter.date(fecha1,fecha2));
// Map.addLayer(nicfi.mean().clip(regionLayer), RGB,'NICFI '+fecha2,false);
    ...
    if(param.classificationArea.useshp){
      ...
      Map.addLayer(regionLayer.style({fillColor: '00000000'}), {},"Region")
    }
    ...
    // }
      yearMosaicSel = yearMosaicSel.updateMask(yearMosaicSel.select('blue_median').gte(0)).updateMask(classArea);
...
//Map.centerObject(-78.778777, 1.804006, 10)
